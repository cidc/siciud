...
//constructor
	...
	//insetar la segunta Agenda de cooperaci贸n
	public boolean insertaAgenda(Requisitos requis){
		...
		try {
			...
			//almacena la informaci贸n de la Justificaci贸n del investigador
			ps.setString(i++, requis.getCompromisos1());
			ps.setString(i++, requis.getCompromisos2());
			//almacena la informaci贸n de la Agenda de actividades del investigador
	...
	/*metodo para actualizar la informaci贸n del archivo que se ha cargado en el sistema*/
	...
	//Determina si la agenda de cooperaci贸n fue almacenada o no
	...
	//Consulta los datos generales de la inscripci贸n (para el envio el mail)
	...
	//Envia la agenda de cooperaci贸n el mail al correo del investigador
	public boolean enviarMail(String idPropuesta,Persona persona){
		...
		try {
			...
			mail.enviar(destino,"Inscripci髇 Convocatoria Movilidad",""+texto);
			////System.out.println("bandera 7");
			Reporte reporteMail=new Reporte(cursor,super.perfil);
			reporteMail.reportar(persona.getNombre(),"Inscripci髇 Convocatoria 2012",destino[0],consMail);
			retorno=true;
	...
	/*public boolean grupoHabilitado(Persona persona){
		...
		if(persona.getDocumento()!=null){
			if(persona.getDocumento().trim().equals("")){
				setMensaje("Es necesario completar la informaci锟絥 de datos personales para poder continuar con la inscripci锟絥");
			}else{
				Connection cn=null;
				PreparedStatement ps=null;
				ResultSet rs=null;
