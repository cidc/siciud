...
//				irA="/grupos/proyectos/ListaGastos.jsp";
			...
			case Parametros.ENVIODATOSWEBSERVICE:
				...
				try {
					...
					System.out.println(calc.saveEntityAsString(objXml.crearPersonaYProyecto(usuario, proyecto)));
					...
					String respWS =null;
					if(!(respWS= wfe.createCasesAsString(objXml.crearCaso(usuario, proyecto, (Integer)sesion.getAttribute("TipoModificacion"), archivoBase64))).equals(null)){
						String idCaso=String.valueOf(objXml.leerIdCaso(respWS));
						wfe.performActivityAsString(objXml.avanzarCaso(idCaso));
						req.setAttribute("visible", true);
						req.setAttribute("estado", true);
						ProyectosInvestigadorDB proyDB=new ProyectosInvestigadorDB(cursor,usuario.getPerfil());
						proyDB.guardarSolicitudBPM(usuario, idCaso, proyecto, String.valueOf(sesion.getAttribute("TipoModificacion")), "/siciud"+nombreArchivo);
