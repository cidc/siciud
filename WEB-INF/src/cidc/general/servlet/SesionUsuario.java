 ...
 * Servlet Class
 * 
 * @web.servlet name="SesionUsuario" display-name="Name for SesionUsuario"
...
public class SesionUsuario extends ServletGeneral {
	...
	public String[] operaciones(HttpServletRequest req, HttpServletResponse resp)
		...
		case 1:
			...
			if (user != null) {
				...
				if (user.isPerfil("8") || user.isPerfil("10")) {
					...
					sesion.setAttribute("listaMisGrupos",
							adminGruposDB.getGruposPersona(user.getIdUsuario()));
				}
		...
		case 3:
			...
			if (!correo.isEmpty()) {
				Integrante integr = usuarioDB.buscarPorCorreo(correo);
				if (integr != null) {
					gestionGrupoDB = new GruposGestionDB(cursor,Parametros.userVisitante);
					GestionGrupos gestion =new GestionGrupos();
					String nickname=gestion.completeid(integr);
					if (gestionGrupoDB.claveInvestigador(String.valueOf(integr.getId()),String.valueOf(integr.getPapel()), nickname)) {
						irA = "/menu.jsp";
