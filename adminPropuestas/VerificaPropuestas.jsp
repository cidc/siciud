...
<html>
<head>
<jsp:useBean id="loginUsuario" class="cidc.general.login.Usuario"
	scope="session" />
...
<body onLoad="mensajeAlert(document.getElementById('msg'));">
			...
			<tr>
				...
				<td><select name="ano" onchange="ajaxNumeros(this)">
						...
						<c:forEach begin="0" items="${requestScope.listaConv}" var="anos">
							<option value='<c:out value="${anos}"/>'
								<c:if test="${sessionScope.ano==anos}">selected</c:if>>
								<c:out value="${anos}" />
							</option>
						</c:forEach>
				</select></td>
				...
				<td><select name="num">
						...
						<c:forEach begin="0" items="${requestScope.listaNum}" var="num">
							<option value='<c:out value="${num}"/>'
								<c:if test="${sessionScope.num==num}">selected</c:if>>
								<c:out value="${num}" />
							</option>
						</c:forEach>
				</select></td>
				...
				<td>Activa <input type="radio" name="activa" value="true"
					<c:if test="${sessionScope.activa==true or sessionScope.activa==null}">checked="checked"</c:if>></td>
				<td>Desactiva <input type="radio" name="activa" value="false"
					<c:if test="${sessionScope.activa==false}">checked="checked"</c:if>></td>
				<%
					}
				%>
				<%
				...
				<td><select name="ano">
						<option value="0">----</option>
						<option value='2009'
							<c:if test="${sessionScope.ano==2009}">selected</c:if>>2009</option>
				</select></td>
				<td>#</td>
				<td><select name="num">
						<option value="0">----</option>
						<option value="5"
							<c:if test="${sessionScope.num==5}">selected</c:if>>5</option>
				</select></td>
				<%
					}
				%>
				<td><img onclick="ir()"
					src='<c:url value="/comp/img/Buscar.gif"/>'></td>
	...
	<c:if test="${requestScope.convEstado!=null}">
		...
		<form name="listado" method="post"
			...
			<table class="tablas" width="98%">
					...
					<tr <c:if test="${st.count mod 2==0}">class="trb"</c:if>>
						...
						<td class="listas" width="25px"><c:forEach begin="0" items="${sessionScope.listaDocOBJ}" var="lista2" varStatus="st">
								<c:if test="${lista.codPropuesta==lista2.codPropuesta}">
									<a
										href="<c:url value="${lista2.url}"/><c:url value="${lista2.docNombre}"/>"><img
										border="0" src='<c:url value="/comp/img/pdf.png"/>'></a>
				...
				<tr>
					<td><img src="<c:url value="/comp/img/Guardar.gif"/>"
						onclick="guardar()"></td>
				</tr>
			</table>
		</form>
	...
	<table>
		<tr>
			<td style="display: none"><iframe name="frameOculto"
					id="frameOculto"></iframe></td>
		</tr>
	</table>
	<form method="post" name="frmAjaxNumero"
		action="<c:url value="/adminPropuestas/Ajax.x"/>">
		<input type="hidden" name="accion" value='7'> <input
			type="hidden" name="dato" value=''>
	</form>
