...
<script>
	...
	function guardar(){
		alert(document.nuevo.codigo.value);
		if(document.nuevo.codigo.selectedIndex==0){
			alert ("Seleccione un proyecto de investigación");
			return false;
		}else{		
			document.nuevo.accion.value='9';
			document.nuevo.codProy.value=document.nuevo.codigo.value;
			document.nuevo.action='<c:url value="/certificaciones/AdminCertificados.x"/>';
...
<body>Actividades de Investigación
<div align="center">
	...
	<input type="hidden" name="idPersona" value='<c:out value="${sessionScope.persona.idPersona}" />'>	
		<table width="100%" align="center">
				...
				<tr>
					...
					<td colspan="2">
						<input type="hidden" name="codProy">						
						<select name="codigo" >
							<option value="0">----------------</option>
							<c:forEach begin="0" items="${sessionScope.listaAct}" var="lista">
				...
				<c:if test='${sessionScope.actividad}'>
				<tr>
				...
				<c:if test='${!sessionScope.actividad}'>
				<tr>
					<td><br><h3 aling="center">No existen Proyectos a su Nombre</h3></td></td>					
				</tr>
				</c:if>
			</table>
