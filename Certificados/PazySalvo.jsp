...
<form name="nuevo" method="post" action='<c:url value="/certificados/llenar.jsp"/>'>
	...
	<input type="hidden" name="idPersona" value='<c:out value="${sessionScope.persona.idPersona}" />'>	
		<table width="100%" align="center">
				...
				<tr>
					<td width="60px" class="renglones" colspan="3"><b>Facultad:</b></td>					
				</tr>
				<tr>
					<td colspan="3">
				...
				<tr>
					...
					<td colspan="1">					
						...
						<c:if test="${sessionScope.persona.papel==1}">Profesor</c:if>
						...
						<c:if test="${sessionScope.persona.papel==5}">Estudiante</c:if>
				...
				<tr>
					<td colspan="3" align="center">En este momento usted se encuentra a paz y salvo por todo concepto en el Centro de Investigaciones.
					Haga click en el boton Generar Cetificado para obtener el documento</td>
				</tr>
				<tr>
			...
			<c:if test='${!sessionScope.paz}'>
				<h3 align="center">Lo sentimos, el certificado no se puede generar debido a que usted no se encuentra a paz y salvo.
						Para solucionar esto por favor comuniquese con el Gestor Académico del CIDC</h3>
			</c:if>
	</form>
</fieldset>
</div>
